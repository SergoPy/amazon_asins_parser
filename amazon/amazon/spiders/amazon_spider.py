...
class AmazonSpider(scrapy.Spider):
    ...
    SP_DEF_COLUMN = 'Brand Def'

    def __init__(self, urls, limit=1, price_filter=None, review_filter=None, rating_filter=None,
                 keywords=None, sp_def_asins='', table_link=None, apikey_file_path=None, cookie=None, **kwargs):
        ...
        self.googlesheets_api = GoogleSheetsApi(self.table_id, apikey_file_path)
    ...
    def get_categories_cords(self) -> tuple:
        cords = {}
        data = {}
        for category in ['TCA', 'LSA', 'LPA', 'RA', 'CA', self.SP_DEF_COLUMN]:
    ...
    def save_product_data(self, asin: str, price: str, reviews: str, rating: str, title: str) -> None:
        category, asin, value = self.get_product_data(asin, price, reviews, rating, title)
    ...
    def write_product_data(self) -> None:
        ...
        self.write_data[sp_def_key] = [[asin] for asin in set(self.sp_variations_asins)]
        for category in self.cords:
            if self.write_data[category]:
                ...
                self.googlesheets_api.update(diapason, self.write_data[category])
    ...
    def get_product_data(self, asin: str, price: str, reviews: str, rating: str, title: str) -> tuple:
        ...
        try:
            ...
            elif self.price_filter and price_value and price_value <= self._float(self.price_filter):
                return 'lpa', asin, price_value
            elif self.review_filter and reviews and self._float(self.review_filter) <= int(reviews.replace(',', '')):
                return 'tca', asin, int(reviews.replace(',', ''))
            elif self.rating_filter and rating and self._float(rating) <= self._float(self.rating_filter):
    ...
    def scrape_pages(self, response):
        ...
        next_page = response.css('div.s-pagination-container').css('a::attr(href)').get()
    ...
    def scrape_item(self, item, search_tag, retry_count=0):
        ...
        if asin and asin not in self.asins_cache and len(asin) >= 10:
            ...
            price = item_data.css('span.a-price').css('span.a-offscreen::text').get()
            review_data = item_data.css('span::attr(aria-label)').getall()
            if review_data:
                try:
                    rating, reviews = review_data[0].split()[0], review_data[1]
                except IndexError:
                    try:
    ...
    def scrape(self, response):
        ...
        advertising_items = []
        advertising_body = response.css('.sg-col-12-of-16 .s-widget-spacing-large')
