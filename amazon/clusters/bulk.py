    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                + ["Ad Group"]
                + ["Product Ad"] * len(skus)
                + ["Product targeting"] * len(asins),
                ["Create"] * total_def_len,
                [data[1].split("|")[0] + f"| {data[2]} | {data[0]} {cmp_ending}"]
                * total_def_len,
    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                + ["Bidding Adjustment"] * 2
                + ["Ad Group"]
                + ["Product Ad"] * len(adv_items)
                + ["Product targeting"] * len(data[0]),
                ["Create"] * total_def_len,
                [data[1].split("|")[0] + f"| Self | {data[0][0]} {cmp_ending}"]
                * total_def_len,
                ...
                none,
                [None] * 4
                + ["Eligible"] * len(adv_items)
                + [None] * (total_def_len - 4 - len(adv_items)),
                none,
                [None] * 3 + [data[3]] + [None] * (total_def_len - 4),
                none,
...
def make_seed_one(seed_data, word, target_asin, tos, bid):
    global cmp_ending
    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                [None] * 3 + [seed_data[1]] + [None] * (total_seed_len - 4),
                none,
                [None] * 3 + [seed_data[1]] * (total_seed_len - 3),
                none,
                [str(date.today()).replace("-", "")] + [None] * (total_seed_len - 1),
                none,
                ...
                none,
                [None] * 4 + ["Eligible"] * len(seed_items) + [None],
                none,
                [None] * 3 + [seed_data[-1]] + [None] * (total_seed_len - 4),
                none,
                [None] * (total_seed_len - 1) + [bid],  # тут кастом бид
                [None] * (total_seed_len - 1) + [word],
                [None] * (total_seed_len - 1) + ["exact"],
                ["Dynamic bids - down only"] * 3 + [None] * (total_seed_len - 3),
                [None, "placementProductPage", "placementTop"]
                + [None] * (total_seed_len - 3),
                [None, 0, tos] + [None] * (total_seed_len - 3),  # тут кастом тос
                none,
                none,
                none,
                none,
...
def make_seed_all(seed_total_data, target_asin, seed_tos_items, seed_bid_items):
    ...
    for word, tos, bid in zip(seed_words, tos_items, bid_items):
        cur_df = make_seed_one(seed_data, word, target_asin, tos, bid)
...
def make_broad_all(broad_total_data, keyword_negatives, phrases_negatives, target_asin):
    ...
    broad_words = [f"'{x}" for x in broad_total_data[4:] if x is not None and x != ""]
    ...
    total_broad_len = (
        ...
        + len(broad_items)
        + len(broad_words)
        + len(keyword_negatives)
        + len(phrases_negatives)
    )
        ...
        np.array(
            [
                ...
                * (len(keyword_negatives) + len(phrases_negatives)),
                ...
                [None] * 3
                + [broad_data[1]] * (1 + len(broad_items) + len(broad_words))
                + [None] * (len(keyword_negatives) + len(phrases_negatives)),  # this
                none,
                none,
                none,
                none,
                ...
                + [None] * (len(keyword_negatives) + len(phrases_negatives)),  # this
                none,
                [str(date.today()).replace("-", "")] + [None] * (total_broad_len - 1),
                none,
                ["MANUAL"] + [None] * (total_broad_len - 1),
                ["enabled"] + [None] * 2 + ["enabled"] * (total_broad_len - 3),
                ["enabled"] * total_broad_len,
                [None] * 3
                + ["enabled"] * (1 + len(broad_items) + len(broad_words))
                + [None] * (len(keyword_negatives) + len(phrases_negatives)),
                [300] + [None] * (total_broad_len - 1),
                [None] * 4
                + broad_items
                + [None] * (total_broad_len - 4 - len(broad_items)),
                none,
                [None] * 4
                + ["Eligible"] * len(broad_items)
                + [None] * (total_broad_len - 4 - len(broad_items)),
                none,
                [None] * 3 + [broad_data[-1]] + [None] * (total_broad_len - 4),
                none,
                [None] * (4 + len(broad_items))
                + [broad_data[-1]] * len(broad_words)
                + [None] * (len(keyword_negatives) + len(phrases_negatives)),
                [None] * (4 + len(broad_items))
                + list(broad_words)
                + list(keyword_negatives)
                + list(phrases_negatives),
                [None] * (4 + len(broad_items))
                + ["Broad"] * len(broad_words)
                + ["negativeExact"] * len(keyword_negatives)
...
def make_auto_neg_all(
    auto_neg_total_data, keyword_negatives, phrases_negatives, target_asin, pte
):
    global cmp_ending
    ...
    total_auto_neg_len = (
        4 + len(auto_neg_items) + len(keyword_negatives) + len(phrases_negatives) + 4
    )
    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                + ["Bidding Adjustment"] * 2
                + ["Ad Group"]
                + ["Product Ad"] * len(auto_neg_items)
                + ["Product Targeting"] * 4
                + ["Campaign Negative Keyword"]
                * (len(keyword_negatives) + len(phrases_negatives)),  # 2 - 70
                ["Create"] * total_auto_neg_len,  # 3 -70
                [title[0] + target_asin + f" {cmp_ending}"]
                * total_auto_neg_len,  # 4 -70
                [None] * 3
                + [auto_neg_data[1]] * (total_auto_neg_len - 3),  # 5 - 70  #this
                none,  # 6
                none,  # 7
                none,  # 8
                none,  # 9
                ...
                [None] * 3
                + [auto_neg_data[1]]
                + [None] * (total_auto_neg_len - 4),  # 11 - 70
                none,  # 12
                [None] * 3
                + [auto_neg_data[1]] * (1 + len(auto_neg_items) + 4)
                + [None]
                * (total_auto_neg_len - 8 - len(auto_neg_items)),  # 13 - 70 #this
                none,  # 14
                [str(date.today()).replace("-", "")]
                + [None] * (total_auto_neg_len - 1),  # 15 - 70
                none,  # 16
                ["AUTO"] + [None] * (total_auto_neg_len - 1),  # 17 -70
                ["enabled"]
                + [None] * 2
                + ["enabled"] * (len(auto_neg_items) + 1)
                + pte
                + ["enabled"]
                * (
                    total_auto_neg_len - 8 - len(auto_neg_items)
                ),  # 18 --------------------------------------------72 -------70
                ["enabled"] * total_auto_neg_len,  # 19 - 70
                [None] * 3
                + ["enabled"] * (1 + len(auto_neg_items))
                + [None] * (total_auto_neg_len - 4 - len(auto_neg_items)),  # 20 - 70
                [300] + [None] * (total_auto_neg_len - 1),  # 21 - 70
                [None] * 4
                + auto_neg_items
                + [None] * (total_auto_neg_len - len(auto_neg_items) - 4),  # 22 - 70
                none,  # 23
                [None] * 4
                + ["Eligible"] * len(auto_neg_items)
                + [None] * (total_auto_neg_len - 4 - len(auto_neg_items)),  # 24 - 70
                none,  # 25
                [None] * 3
                + [auto_neg_data[-1]]
                + [None] * (total_auto_neg_len - 4),  # 26  -70
                none,  # 27
                [None] * (4 + len(auto_neg_items))
                + bids
                + [None] * (total_auto_neg_len - 8 - len(auto_neg_items)),  # 28 - 70
                ...
                + ["negativeExact"] * len(keyword_negatives)
                + ["negativePhrase"]
                * len(phrases_negatives),  # 30 - 70 tuuuuuuuuuuuuut
                ["Dynamic bids - down only"] * 3
                + [None] * (total_auto_neg_len - 3),  # 31 - 70 tiuuuuuuuuuuuuuuuuuuuut
                [None, "placementProductPage", "placementTop"]
                + [None] * (total_auto_neg_len - 3),  # 32 - 70
                [None, 0, 0] + [None] * (total_auto_neg_len - 3),  # 33 - 70
                [None] * (4 + len(auto_neg_items))
                + ["close-match", "loose-match", "substitutes", "complements"]
                + [None] * (total_auto_neg_len - 8 - len(auto_neg_items)),  # 34?
                none,
                none,
                none,
                none,
...
def make_auto_all(auto_total_data, target_asin, pte):
    ...
    # print(f"we in make_auto_all title:{title}")

    none = [None] * total_auto_len

    # print(f"we in make_auto_all:{auto_items}")
    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                none,
                [None] * (4 + len(auto_items))
                + bids
                + [None] * (total_auto_len - 8 - len(auto_items)),  # 28 - 70
                none,
                none,
...
def make_exact_part(
    exact_data, words_part, number="", remove_campaign=0, target_asin="", tos=0, bid=0
):
    global cmp_ending
    # print(f"exact_data: {exact_data}")
    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                [None] * 3 + [ad_group] + [None] * (total_exact_len - 4),
                none,
                [None] * 3 + [ad_group] * (total_exact_len - 3),
                none,
                [str(date.today()).replace("-", "")] + [None] * (total_exact_len - 1),
                none,
                ...
                [None] * 4 + exact_items + [None] * len(words_part),
                none,
                [None] * 4 + ["Eligible"] * len(exact_items) + [None] * len(words_part),
                none,
                [None] * 3 + [exact_data[-1]] + [None] * (total_exact_len - 4),
                none,
                [None] * (total_exact_len - len(words_part)) + list(bid),  # тут кастом бид
                ...
                [None, 0, tos] + [None] * (total_exact_len - 3),
                none,
                none,
                none,
                none,
...
def make_exact_all(
    ...
    if exact_tos_list != None and exact_bid_list != None:
        ...
        # print(f"exact_tos_list[4:]: {exact_tos_list}")
        
        
        if len(exact_tos_list) == 0 or exact_tos_list == "" or exact_tos_list == " ":
            tos_items = '0'
        else:
            tos_items = exact_tos_list
        bid_items = [x if x != '' else exact_data[3]  for x in exact_bid_list[4:]]

        # print(f"tos_items[4:]: {tos_items}")
        # print(f"bid_items[4:]: {bid_items}")
        ...
        all_exact_dfs = []
        tos = '0'
        bid = [exact_data[3] for x in exact_words]
        cur_df = make_exact_part(
                    exact_data,
                    exact_words,
                    remove_campaign=remove_campaign,
...
def make_exact_all_list(
    exact_total_data_list, target_asin, exact_tos_list=None, exact_bid_list=None
):
    if exact_tos_list != None and exact_bid_list != None:
        # print(f"exact_tos_list: {exact_tos_list}")
        # print(f"exact_bid_list: {exact_bid_list}")
        # print(f"exact_total_data_list: {exact_total_data_list}")
        exact_data_sorted = [tuple(x) for x in exact_total_data_list]
        # print(f"exact_data_sorted: {exact_data_sorted}")
        all_exact_lists_dfs = []
        i = 0
        for k, exact_tos, exact_bid in zip(
            exact_data_sorted, exact_tos_list, exact_bid_list
        ):
        ...
        # print(f"e_exact_tos_list: {exact_tos_list}")
        # print(f"e_exact_bid_list: {exact_bid_list}")
        # print(f"e_exact_total_data_list: {exact_total_data_list}")
        exact_data_sorted = [tuple(x) for x in exact_total_data_list]
        # print(f"e_exact_data_sorted: {exact_data_sorted}")
        all_exact_lists_dfs = []
        ...
        pd.DataFrame(
            np.array(
                [
                    ...
                    [None] * 4 + pat_items + [None] * len(pat_words),
                    none,
                    [None] * 4
                    + ["Eligible"] * len(pat_items)
                    + [None] * len(pat_words),
                    none,
                    [None] * 3 + [pat_data[-1]] + [None] * (total_pat_len - 4),
                    none,
                    [None] * (total_pat_len - len(pat_words))
                    + [pat_data[-1]] * len(pat_words),
                    none,
                    [None] * (total_pat_len - len(pat_words))
                    + ["exact"] * len(pat_words),
...
def make_pat_all_list(pat_total_data, target_asin, first_group=True):
    ...
    return all_pat_words, pd.concat(all_pat_lists_dfs).reset_index().drop(
        "index", axis=1
    )
...
def make_category_all(category_total_data, pat_words, remove_campaign, target_asin):
    ...
    category_words = [
        'category="' + x + '"'
        for x in category_total_data[4:]
    ...
    return pd.DataFrame(
        np.array(
            [
                ...
                + ["Bidding Adjustment"] * 2
                + ["Ad Group"]
                + ["Product Ad"] * len(category_items)
                + ["Product targeting"] * len(category_words)
                + ["Negative Product Targeting"] * len(pat_words),
                ["Create"] * total_category_len,
                [title[0] + target_asin + f" {cmp_ending}"] * total_category_len,
                [None] * 3 + [category_data[1]] * (total_category_len - 3),
                none,
                none,
                none,
                none,
                ...
                [None] * 3 + [category_data[1]] + [None] * (total_category_len - 4),
                none,
                [None] * 3 + [category_data[1]] * (total_category_len - 3),
                none,
                [str(date.today()).replace("-", "")]
                ...
                [None] * 4
                + category_items
                + [None] * (len(category_words) + len(pat_words)),
                none,
                [None] * 4
                + ["Eligible"] * len(category_items)
                + [None] * (len(category_words) + len(pat_words)),
                none,
                [None] * 3 + [category_data[-1]] + [None] * (total_category_len - 4),
                none,
                [None] * (total_category_len - len(category_words) - len(pat_words))
                + [category_data[-1]] * len(category_words)
                + [None] * len(pat_words),
                none,
                none,
...
def google_sheets_bulk(table_link, campaign_data, cmp_end, campaign_2_data):
    ...
    df_total_general = get_data_frame(API_KEY, SPREADSHEET_ID, range_name_general)
    ...
    if len(brand_defense_list) > 0 and len(brand_defense_list) > 0:
        for brand_defense_list_item in brand_defense_list:
            if len(brand_defense_list_item) > 4 and any(brand_defense_list_item[4]):
                data = [
                    brand_defense_list_item[2],
                    brand_defense_list_item[0].split("(")[0],
                    "Brand Defense",
                    brand_defense_list_item[3],
                ]
    ...
    if (
        len(seed_total_data) > 0
        and len(seed_tos_list) > 0
        and len(seed_bid_list) > 0
        and len(advertised_asins_list) > 0
    ):
        for seed_total_data_item, target_asin, seed_tos_items, seed_bid_items in zip(
            seed_total_data, advertised_asins_list, seed_tos_list, seed_bid_list
        ):
            if any(seed_total_data_item[4:]) and seed_total_data_item[2]:
                seed_all = make_seed_all(
                    seed_total_data_item, target_asin[4], seed_tos_items, seed_bid_items
                )
                table_create_params.append(seed_all)
    ...
    if len(broad_total_data) > 0:
        ...
        if len(advertised_asins_list) < len(broad_total_data):
            advertised_asins_list_prime = [advertised_asins_list[0]] * len(
                broad_total_data
            )
        else:
    ...
    if len(words_total_data) > 0:
        ...
        for words_total_data_item, keyword_negatives, target_asin in zip(
            words_total_data, keyword_negatives_list_prime, advertised_asins_list
        ):
            # print(f"keyword_negatives: {keyword_negatives}")
    ...
    if len(auto_close_neg_total_data) > 0:
        if len(keyword_negatives_list) < len(auto_close_neg_total_data):
            ...
            keyword_negatives_list_prime = keyword_negatives_list
        for auto_close_neg_total_data_item, keyword_negatives, target_asin in zip(
            auto_close_neg_total_data,
            keyword_negatives_list_prime,
            advertised_asins_list,
        ):
            if len(auto_close_neg_total_data_item):
                auto_close_neg_all = make_auto_neg_all(
                    auto_close_neg_total_data_item,
                    keyword_negatives,
                    phrases_negatives,
                    target_asin[4],
    ...
    if len(auto_close_total_data) > 0:
        for auto_close_total_data_item, target_asin in zip(
            auto_close_total_data, advertised_asins_list
        ):
            if len(auto_close_total_data_item):
                auto_close_all = make_auto_all(
                    auto_close_total_data_item,
                    target_asin[4],
                    ["enabled", "paused", "paused", "paused"],
                )
    ...
    if len(auto_loose_neg_total_data) > 0:
        if len(keyword_negatives_list) < len(auto_loose_neg_total_data):
            ...
            keyword_negatives_list_prime = keyword_negatives_list
        for auto_loose_neg_total_data_item, keyword_negatives, target_asin in zip(
            auto_loose_neg_total_data,
            keyword_negatives_list_prime,
            advertised_asins_list,
        ):
            if len(auto_loose_neg_total_data_item):
                auto_loose_neg_all = make_auto_neg_all(
                    auto_loose_neg_total_data_item,
                    keyword_negatives,
                    phrases_negatives,
                    target_asin[4],
    ...
    if len(auto_loose_total_data) > 0:
        for auto_loose_total_data_item, target_asin in zip(
            auto_loose_total_data, advertised_asins_list
        ):
            if len(auto_loose_total_data_item):
                auto_loose_all = make_auto_all(
                    auto_loose_total_data_item,
                    target_asin[4],
                    ["paused", "enabled", "paused", "paused"],
                )
    ...
    if len(auto_subs_neg_total_data) > 0:
        if len(keyword_negatives_list) < len(auto_subs_neg_total_data):
            ...
            keyword_negatives_list_prime = keyword_negatives_list
        for auto_subs_neg_total_data_item, keyword_negatives, target_asin in zip(
            auto_subs_neg_total_data,
            keyword_negatives_list_prime,
            advertised_asins_list,
        ):
    ...
    if len(auto_subs_total_data) > 0:
        for auto_subs_total_data_item, target_asin in zip(
            auto_subs_total_data, advertised_asins_list
        ):
            if len(auto_subs_total_data_item):
                auto_subs_all = make_auto_all(
                    auto_subs_total_data_item,
                    target_asin[4],
                    ["paused", "paused", "enabled", "paused"],
                )
    ...
    if len(auto_compl_neg_total_data) > 0:
        if len(keyword_negatives_list) < len(auto_compl_neg_total_data):
            ...
            keyword_negatives_list_prime = keyword_negatives_list
        for auto_compl_neg_total_data_item, keyword_negatives, target_asin in zip(
            auto_compl_neg_total_data,
            keyword_negatives_list_prime,
            advertised_asins_list,
        ):
            if len(auto_compl_neg_total_data_item):
                auto_compl_neg_all = make_auto_neg_all(
                    auto_compl_neg_total_data_item,
                    keyword_negatives,
                    phrases_negatives,
                    target_asin[4],
    ...
    if len(auto_compl_total_data) > 0:
        for auto_compl_total_data_item, target_asin in zip(
            auto_compl_total_data, advertised_asins_list
        ):
            # print(f"auto_compl_total_data_item: {auto_compl_total_data_item}")
            # print(f"target_asin: {target_asin}")
            if len(auto_compl_total_data_item):
                auto_compl_all = make_auto_all(
                    auto_compl_total_data_item,
                    target_asin[4],
                    ["paused", "paused", "paused", "enabled"],
                )
...
def split_and_upload_large_file(table_name, sht1, endings):
    ...
    if sqr_pd > 9999999:
        split_and_upload_large_file(
            f"{table_name}_{endings[0]}", sht1, ["part_3", "part_4"]
        )
    else:
        sht1.values_update(
            ...
            params={"valueInputOption": "USER_ENTERED"},
            body={
                "values": list(csv.reader(open(f"media/{table_name}_{endings[0]}.csv")))
            },
        )
    ...
    if sqr_pd > 9999999:
        split_and_upload_large_file(
            f"{table_name}_{endings[1]}", sht1, ["part_5", "part_6"]
        )
    else:
        sht1.values_update(
            ...
            params={"valueInputOption": "USER_ENTERED"},
            body={
                "values": list(csv.reader(open(f"media/{table_name}_{endings[1]}.csv")))
            },
        )
